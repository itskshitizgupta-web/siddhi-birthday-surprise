...
@keyframes birthdayPulse{
  0%,100%{transform:scale(1);filter:drop-shadow(0 0 0 rgba(255,120,170,0))}
  50%{transform:scale(1.035);filter:drop-shadow(0 12px 28px rgba(214,63,118,.22))}
}
...
# -----------------------------
    ...
    <div style="text-align:center;font:700 clamp(32px,6vw,58px) 'DM Serif Display';color:#d24c7c;text-shadow:0 7px 25px rgba(210,76,124,.18);margin:18px 0 12px;">Do uh like me, Siddhi ji? 💗</div>
    <br>
    <div class="sub">
    Choose honestly. The next chapter opens only when you choose “I LIKE YOU”. 🌷
    </div>
    """, unsafe_allow_html=True)

    a,b = st.columns(2)
    with a:
        if st.button("💗 I LIKE YOU", key="like"):
            st.session_state.liked = True
            st.session_state.stage = 4
            st.rerun()
    with b:
        if st.button("😶 I DON'T LIKE YOU", key="dont"):
            st.session_state.no_count += 1
            st.warning("Ohooo 😭😂 The final chapter is still locked. Maybe think about that answer again, Siddhi ji. 💗")
...
# -----------------------------
    ...
    st.markdown(
        ...
        '<div style="font:700 clamp(48px,10vw,96px) \'DM Serif Display\'; '
        'color:#d63f76; line-height:.98; '
        'text-shadow:0 8px 30px rgba(214,63,118,.22), 0 0 45px rgba(255,135,178,.30); '
        'animation: birthdayPulse 2s ease-in-out infinite;">'
        'HAPPY BIRTHDAY<br>SIDDHI JEEE! 🎂💗'
        '</div>'
        '<div style="font:700 14px Manrope; letter-spacing:4px; color:#a45a78; margin-top:15px;">'
        'THIS FINAL PAGE IS ALL YOURS ✨'
        '</div></div>',
        unsafe_allow_html=True
    )
    st.markdown('<div class="big pulse">YOU UNLOCKED THE SURPRISE. ❤️</div><br>', unsafe_allow_html=True)
